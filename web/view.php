...
else
{
	...
	if($amount==300)
	{
		...
		<ul id="flexiselDemo3">
								<li>
								<?php
							 	 $o="SELECT * FROM reister LEFT JOIN profile on reister.id=profile.profile_id 
								where profile.status is null order by reister.id desc limit 0,10";
								$ret=select($o);
								while($t=mysqli_fetch_array($ret))
								{extract($t);
								?>
									<div class="col-md-3 biseller-column">
										<a href="groom_profile1.php?id=<?=$t[0]?>">
	...
	?>
		...
		<ul id="flexiselDemo3">
								...
								<?php
							 	$o="SELECT * FROM reister LEFT JOIN profile on reister.id=profile.profile_id 
								where profile.status is null order by reister.id desc limit 0,50";
								$ret=select($o);
								while($t=mysqli_fetch_array($ret))
								{extract($t);
								?>
	...
	?>
		...
		<ul id="flexiselDemo3">
								...
								<?php
							 	$o="SELECT * FROM reister LEFT JOIN profile on reister.id=profile.profile_id 
								where profile.status is null order by reister.id desc limit 0,100";
								$ret=select($o);
								while($t=mysqli_fetch_array($ret))
								{extract($t);
								?>
									<div class="col-md-3 biseller-column">
										<a href="groom_profile.php?id=<?=$t[0]?>">
											<div class="profile-image">
												<img src="images/<?=$img?>" style="height:320px" class="img-responsive" alt="profile image">
												<div class="agile-overlay">
												<h4>Profile ID: 458deng<?=$id?></h4>
